...
// Data source for the Black Follow-ups panel. The admin page uses the staff
// endpoints; the employee portal passes the /api/eportal/* equivalents (scoped to
// owned POs) with canSend=true so the same panel serves both. Keeping this an
// adapter means the 700-line panel below is written once.
export interface BlackFollowupsAdapter {
  list: (limit?: number) => Promise<BlackFollowupResponse>;
  history: (params: { signal?: string; outcome?: string; supplier_po_no?: string; limit?: number }) => Promise<FollowupHistoryResponse>;
  command: (supplier_po_no: string, instruction: string, send?: boolean) => Promise<BlackFollowupCommandResult>;
  // When set, overrides the role check for the Send button (employees may send on
  // POs they own). When undefined, the panel falls back to the manager+ role gate.
  canSend?: boolean;
}

const ADMIN_BLACK_ADAPTER: BlackFollowupsAdapter = {
  list: (limit = 100) => api.getBlackFollowups(limit),
  history: (params) => api.getFollowupHistory(params),
  command: (po, instruction, send = false) => api.blackFollowupCommand(po, instruction, send),
};
...
export function BlackFollowupsPanel({
  ...
  const loadHistory = useCallback(() => {
    ...
    setError(null);
    adapter
      .history({ signal: "BLACK", limit: 200 })
    ...
    <div className="page-stack">
      ...
      {view === "active" ? (
        <>
          ...
          {selected && (
            <DetailDrawer item={selected} adapter={adapter} onClose={() => setSelectedPo(null)} onSent={load} />
          )}
        </>
      ) : (
...
function DetailDrawer({
  ...
  const draft = async () => {
    ...
    setNote(null);
    try {
      const r = await adapter.command(item.supplier_po_no, cmd.trim(), false);
      setPreview(r);
      setView("html");
      setFeedback(null);
    } catch (e) {
