...
function MailQueueBase({
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="mail-pane-header flex flex-col justify-center px-3 py-2">
        <div className="mail-search relative">
          <Search className="pointer-events-none absolute left-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-brand-muted" />
          <input
            value={searchInput}
            onChange={(e) => onSearchChange(e.target.value)}
            placeholder="Search subject or sender"
            className="w-full border-0 bg-transparent py-2 pl-8 pr-3 text-sm outline-none"
          />
        </div>
      </div>

      <div className="flex gap-1.5 overflow-x-auto border-b border-brand-border/70 px-3 py-2.5 scrollbar-thin">
        {tabs.map((t) => {
          ...
          <>
            ...
            {visible < mails.length && (
              <button
                ...
                className="m-3 w-[calc(100%-1.5rem)] rounded-lg border border-brand-border bg-white py-2 text-xs font-semibold text-brand-muted hover:bg-gray-50"
              >
