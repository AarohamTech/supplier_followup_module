...
export default function StaffAsnsPage() {
  ...
  const loadSummary = useCallback(async () => {
    try {
      setSummary(await api.asnSummary());
    } catch {
      /* non-fatal */
    }
  }, []);
  ...
  const loadList = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.listAsns({ tab: tab || undefined, search: search || undefined });
      setItems(res.items);
      setError(null);
    } catch (e) {
  ...
  return (
    <div className="page-stack">
      <PageHeader
        title="Shipments (ASN)"
        description="Advance Shipping Notices submitted by suppliers."
        icon={Truck}
        actions={
          <input
            value={search}
      ...
      {selected && <AsnDrawer asn={selected} staff onClose={() => setSelected(null)} onUpdated={onUpdated} />}
    </div>
  );
}
