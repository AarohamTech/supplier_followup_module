...
function DetailDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      {/* Panel */}
      <div
        className={`absolute inset-y-0 right-0 flex w-full max-w-xl flex-col bg-white shadow-2xl transition-transform duration-300 ${
          shown ? "translate-x-0" : "translate-x-full"
        }`}
      >
        {/* Header */}
        <div className="flex items-start justify-between gap-3 border-b border-brand-border bg-brand-surface px-4 py-3">
          <div className="min-w-0">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-brand-muted">
              <span>{item.material_count} material(s)</span>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {item.commitment_captured ? (
              <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-semibold text-emerald-700">
                <CheckCircle2 className="h-3.5 w-3.5" /> Commitment received
              </span>
            ) : (
              <span className="inline-flex items-center gap-1.5 rounded-full bg-red-100 px-2.5 py-1 text-xs font-semibold text-signal-red">
                <Clock className="h-3.5 w-3.5" /> AI chasing for commitment
              </span>
            )}
            <span className="text-[11px] text-brand-muted">
              {item.committed_count}/{item.material_count} materials committed
            </span>
            <span className="ml-auto inline-flex items-center gap-2 text-[11px] text-brand-muted">
          ...
          <div className="rounded-xl border border-violet-200 bg-violet-50/40 p-3">
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-1.5 rounded-md border border-violet-300 bg-white px-3 py-2 text-sm font-medium text-violet-700 hover:bg-violet-50 disabled:opacity-50"
              >
            ...
            {preview && (
              <div className="animate-slide-down mt-2 rounded-lg border border-violet-200 bg-white p-3">
